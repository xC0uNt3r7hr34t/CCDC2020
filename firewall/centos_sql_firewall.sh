...
read -p 'IP of SQL Server: ' WEB_SERVER

echo SQL Server: $WEB_SERVER
...
do
	iptables -A INPUT -p tcp -s $ip --dport 3306 -m conntrack -ctstate NEW,ESTABLISHED -j ACCEPT
	iptables -A OUTPUT -p tcp -d $ip --sport 3306 -m conntrack --ctstate ESTABLISHED -j ACCEPT
	iptables -A INPUT -p tcp -s $ip --dport 5432 -m conntrack -ctstate NEW,ESTABLISHED -j ACCEPT
	iptables -A OUTPUT -p tcp -d $ip --sport 5432 -m conntrack --ctstate ESTABLISHED -j ACCEPT
done
